...
import type { Lang } from "../_shared/i18n.ts";
import { buildSystemPrompt, type RetrievedDoc } from "../_shared/system-prompt.ts";
...
async function embedQuery(text: string): Promise<number[]> {
  ...
  if (!resp.ok) throw new Error(`embedQuery ${resp.status}: ${await resp.text()}`);
...
Deno.serve(async (req) => {
  try {
    ...
    // RAG: embed the user question + retrieve top-K relevant chunks
    let retrieved: RetrievedDoc[] = [];
    try {
      const queryEmbedding = await embedQuery(body.message);
      const { data: matches, error: matchErr } = await client.rpc("match_kb_docs", {
        query_embedding: queryEmbedding,
        match_count: RETRIEVAL_TOP_K,
        filter_lang: "pt-br",
      });
      if (matchErr) {
        console.error("match_kb_docs failed:", matchErr);
      } else {
        retrieved = (matches ?? []) as RetrievedDoc[];
      }
    } catch (e) {
      console.error("retrieval failed (proceeding without):", e);
    }

    // Build prompt
    const system = buildSystemPrompt(body.lang as Lang, retrieved);
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        let full = "";
        // Build a leak-detection corpus that the model should NOT echo:
        // system prompt itself + all retrieved chunks.
        const leakCorpus = system + "\n" + retrieved.map((r) => r.content).join("\n");
        try {
          for await (
            const chunk of callLLM({
              system,
              history,
              userMessage,
            ...
            full += chunk;
            const leak = detectLeak(full, leakCorpus);
            if (leak.leak) {
              const replacement =
                "Desculpe, não posso responder essa pergunta. Posso ajudar com outra coisa?";
              controller.enqueue(
          ...
          controller.enqueue(
            encoder.encode(
              `event: done\ndata: ${
                JSON.stringify({ message_id: inserted?.id ?? null })
              }\n\n`,
            ),
          );
        } catch (e) {
